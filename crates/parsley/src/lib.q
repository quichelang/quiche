# Parsify - Argument Parser for Quiche
#
# Features:
# - Default arguments
...
def create_parse_result() -> ParseResult:
    return ParseResult(flags=HashMap.new(), options=HashMap.new(), positionals=[], errors=[])

def get_flag(result: ref[ParseResult], name: String) -> bool:
    key = deref(name)
    match result.flags.get(ref(key)):
        case Some(v):
            return deref(v)
        case None:
            return False

def get_option(result: ref[ParseResult], name: String) -> String:
    key = deref(name)
    match result.options.get(ref(key)):
        case Some(v):
            return deref(v).clone()
        case None:
            return ""

def has_errors(result: ref[ParseResult]) -> bool:
...
class Parser:
    ...
    description: String
    specs: Vec[ArgSpec]
    positional_specs: Vec[ArgSpec]

def create_parser(program_name: String, description: String) -> Parser:
    specs: Vec[ArgSpec] = []
    ...
    return Parser(
        ...
        description=description,
        specs=specs,
        positional_specs=positionals
    )

def add_flag(parser: mutref[Parser], name: String, short: String, description: String):
    spec = new_flag(name, short, description)
    parser.specs.push(spec)

def add_option(parser: mutref[Parser], name: String, short: String, description: String, default: String):
    spec = new_option(name, short, description, default, False)
    parser.specs.push(spec)

def add_required_option(parser: mutref[Parser], name: String, short: String, description: String):
    spec = new_option(name, short, description, "", True)
    parser.specs.push(spec)

def add_positional(parser: mutref[Parser], name: String, description: String):
    spec = new_positional(name, description, True)
    parser.positional_specs.push(spec)

def add_optional_positional(parser: mutref[Parser], name: String, description: String):
    spec = new_positional(name, description, False)
    parser.positional_specs.push(spec)

def parse(parser: ref[Parser], args: Vec[String]) -> ParseResult:
    result = create_parse_result()

    # Initialize defaults for flags
    for spec in parser.specs.iter():
        if spec.is_flag:
    ...
    pi = 0
    while pi < parser.positional_specs.len():
        spec = parser.positional_specs[pi].clone()
...
def help_text(parser: ref[Parser]) -> String:
    out = parser.program_name.clone() + "\n"
    if parser.description != "":
        out = out + parser.description.as_str() + "\n"
    out = out + "\nUSAGE:\n"
    out = out + "    " + parser.program_name.as_str()

    # Show positionals in usage
    for spec in parser.positional_specs.iter():
        if spec.required:
    ...
    if parser.positional_specs.len() > 0:
        out = out + "ARGS:\n"
        for spec in parser.positional_specs.iter():
