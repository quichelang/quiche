...
impl Codegen {
    ...
    /// Check if self is mutated in a list of statements
    pub(crate) fn is_self_mutated(&self, stmts: &[ast::QuicheStmt]) -> bool {
        for stmt in stmts {
            match stmt {
                ast::QuicheStmt::Assign(a) => {
                    for target in &a.targets {
                        match target {
                            ast::QuicheExpr::Attribute { value, .. } => {
                                if self.is_self_expr(value) {
                                    return true;
                                }
                            }
                            ast::QuicheExpr::Subscript { value, .. } => {
                ...
                ast::QuicheStmt::If(i) => {
                    if self.is_self_mutated(&i.body) || self.is_self_mutated(&i.orelse) {
                        return true;
                    }
                }
                ast::QuicheStmt::While(w) => {
                    if self.is_self_mutated(&w.body) || self.is_self_mutated(&w.orelse) {
                        return true;
                    }
                }
                ast::QuicheStmt::For(f) => {
                    if self.is_self_mutated(&f.body) || self.is_self_mutated(&f.orelse) {
                        return true;
                    }
                }
                ast::QuicheStmt::Match(m) => {
                    for case in &m.cases {
                        if self.is_self_mutated(&case.body) {
                            return true;
                        }
                    }
                }
                ...
                ast::QuicheStmt::AnnAssign(a) => {
                    if let ast::QuicheExpr::Attribute { value, .. } = &*a.target {
                        if self.is_self_expr(value) {
                            return true;
                        }
                    }
    ...
    fn is_self_expr(&self, expr: &ast::QuicheExpr) -> bool {
        matches!(expr, ast::QuicheExpr::Name(n) if n == "self")
    }

    /// Check if an expression is `self` or `self.field` or `self.field[i]` etc.
    fn is_nested_self(&self, expr: &ast::QuicheExpr) -> bool {
        match expr {
            ast::QuicheExpr::Name(n) => n == "self",
            ast::QuicheExpr::Attribute { value, .. } => self.is_nested_self(value),
            ast::QuicheExpr::Subscript { value, .. } => self.is_nested_self(value),
            _ => false,
        }
    }

    /// Check if an expression is a mutating method call on self
    fn is_mutating_call(&self, expr: &ast::QuicheExpr) -> bool {
        if let ast::QuicheExpr::Call { func, .. } = expr {
