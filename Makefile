...
STAGE1_BIN := $(STAGE1_TARGET_DIR)/debug/quiche-self

# Stage 2 (built with stage 1)
STAGE2_TARGET_DIR := target/stage2
STAGE2_BIN := $(STAGE2_TARGET_DIR)/debug/quiche-self
...
stage1: stage0
	...
	QUICHE_STAGE=stage1 QUICHE_BOOTSTRAP_BIN=$(abspath $(HOST_BIN)) CARGO_TARGET_DIR=$(STAGE1_TARGET_DIR) cargo build -p quiche-self
	@ln -sf $(STAGE1_BIN) stage1

stage2: stage1
	@echo "Building Stage 2 (Stage 1 -> Self)..."
	QUICHE_STAGE=stage2 QUICHE_BOOTSTRAP_BIN=$(abspath $(STAGE1_BIN)) CARGO_TARGET_DIR=$(STAGE2_TARGET_DIR) cargo build -p quiche-self
	@ln -sf $(STAGE2_BIN) stage2

verify: stage2
	@echo "Verifying Stage 1 output matches Stage 2 output..."
	python3 verify.py diff $(STAGE1_TARGET_DIR)/debug/build/quiche-self-*/out $(STAGE2_TARGET_DIR)/debug/build/quiche-self-*/out
